...
import { Alert, FlatList, Pressable, StyleSheet, Text, View, type NativeSyntheticEvent, type NativeScrollEvent } from "react-native";
import { SafeAreaView } from "react-native-safe-area-context";
import { requestPermission } from "../lib/notifications";
import { disableCustomAlarm, enableCustomAlarm, getCustomAlarm, type RepeatMode } from "../lib/customAlarm";
...
function repeatSummary(mode: RepeatMode, days: number[]): string {
  if (mode === "once") return "once";
  if (mode === "everyday") return "every day";
  if (mode === "weekdays") return "on weekdays";
  if (days.length === 0) return "on selected days";
  return `on ${days.slice().sort().map((d) => DAY_NAMES[d]).join(", ")}`;
}
...
export default function CustomAlarmScreen() {
  ...
  useEffect(() => {
    getCustomAlarm().then((state) => {
      if (state) {
        const hour12 = state.hour % 12 === 0 ? 12 : state.hour % 12;
        setHourIndex(HOURS.indexOf(hour12));
        setMinuteIndex(MINUTES.indexOf(state.minute));
        setAmpmIndex(state.hour >= 12 ? 1 : 0);
        setRepeatMode(state.repeatMode);
        setCustomDays(state.customDays);
  ...
  async function handleSave() {
    ...
    await enableCustomAlarm(hour24, minute, repeatMode, customDays);
    setEnabled(true);
    Alert.alert(
      "Alarm set",
      `You'll be woken up ${repeatSummary(repeatMode, customDays)} at ${hour12}:${String(minute).padStart(2, "0")} ${AMPM[ampmIndex]}.`
    );
  }

  async function handleDisable() {
    await disableCustomAlarm();
    setEnabled(false);
  }
  ...
  return (
    <SafeAreaView style={styles.root} edges={["bottom"]}>
      <View style={styles.content}>
        <Text style={styles.title}>Set your own alarm time</Text>
        <Text style={styles.subtitle}>
          Pick any time you want — it doesn't have to match Bruno's real schedule.
        </Text>
        ...
        <Pressable style={styles.primaryButton} onPress={handleSave}>
          <Text style={styles.primaryButtonText}>{enabled ? "Update alarm" : "Set alarm"}</Text>
        </Pressable>

        {enabled && (
          <Pressable style={styles.secondaryButton} onPress={handleDisable}>
            <Text style={styles.secondaryButtonText}>Turn off custom alarm</Text>
          </Pressable>
        )}
      </View>
    </SafeAreaView>
